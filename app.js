...
// POST /users/link-shortId/:id
app.post('/users/link-shortId/:id', async (req, res) => {
    const { id } = req.params; // Retrieve id from route parameter
    const { shortId } = req.body; // Retrieve shortId from request body
    const userId = id; // Use the id parameter directly as userId (assuming it's the same)

    try {
        // Simulate inserting into database (since we don't have actual db connection in this example)
